...
class Mail
{
    ...
    public function send(?array $options = null): void
    {
        switch ($this->transport) {
            ...
            case 'sendgrid':
                ...
                break;
            // ========== MailFlow邮箱API by xkatld 开始 ==========
            // @link https://github.com/xkatld/FOSSBilling-Patch
            // @version v1.0.0
            case 'mailflow':
                $this->__sendViaMailflow($options);
                return;
    ...
    private function __sendViaMailflow(array $options): void
    {
        ...
        $toAddresses = array_map(fn($addr) => $addr->getAddress(), $this->email->getTo());
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, [
            'X-API-Key: ' . $options['mailflow_key'],
            'Content-Type: application/json',
            'Accept: application/json',
        ]);
        curl_setopt($ch, CURLOPT_TIMEOUT, intval($options['mailflow_timeout'] ?? 30));
