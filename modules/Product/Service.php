...
class Service implements InjectionAwareInterface
{
    ...
    {
        $sql = 'SELECT id, title FROM product WHERE 1';
        $type = $data['type'] ?? null;
    ...
    public function copyProduct(\Model_Product $model)
    {
        $oldPayment = $this->di['db']->load('ProductPayment', $model->product_payment_id);
        $newPayment = $this->di['db']->dispense('ProductPayment');
        $newPayment->type = $oldPayment->type;
        $newPayment->once_price = $oldPayment->once_price;
        $newPayment->once_setup_price = $oldPayment->once_setup_price;
        $newPayment->w_price = $oldPayment->w_price;
        $newPayment->w_setup_price = $oldPayment->w_setup_price;
        $newPayment->w_enabled = $oldPayment->w_enabled;
        $newPayment->m_price = $oldPayment->m_price;
        $newPayment->m_setup_price = $oldPayment->m_setup_price;
        $newPayment->m_enabled = $oldPayment->m_enabled;
        $newPayment->q_price = $oldPayment->q_price;
        $newPayment->q_setup_price = $oldPayment->q_setup_price;
        $newPayment->q_enabled = $oldPayment->q_enabled;
        $newPayment->b_price = $oldPayment->b_price;
        $newPayment->b_setup_price = $oldPayment->b_setup_price;
        $newPayment->b_enabled = $oldPayment->b_enabled;
        $newPayment->a_price = $oldPayment->a_price;
        $newPayment->a_setup_price = $oldPayment->a_setup_price;
        $newPayment->a_enabled = $oldPayment->a_enabled;
        $newPayment->bia_price = $oldPayment->bia_price;
        $newPayment->bia_setup_price = $oldPayment->bia_setup_price;
        $newPayment->bia_enabled = $oldPayment->bia_enabled;
        $newPayment->tria_price = $oldPayment->tria_price;
        $newPayment->tria_setup_price = $oldPayment->tria_setup_price;
        $newPayment->tria_enabled = $oldPayment->tria_enabled;
        $paymentId = $this->di['db']->store($newPayment);

        $newTitle = $model->title . ' - Copy';
        $newSlug = $this->di['tools']->slug($newTitle) . '-' . random_int(1000, 9999);

        $newProduct = $this->di['db']->dispense('Product');
        $newProduct->product_payment_id = $paymentId;
        $newProduct->product_category_id = $model->product_category_id;
        $newProduct->form_id = $model->form_id;
        $newProduct->title = $newTitle;
        $newProduct->slug = $newSlug;
        $newProduct->description = $model->description;
        $newProduct->unit = $model->unit;
        $newProduct->active = 1;
        $newProduct->status = \Model_Product::STATUS_DISABLED;
        $newProduct->hidden = $model->hidden;
        $newProduct->is_addon = $model->is_addon;
        $newProduct->setup = $model->setup;
        $newProduct->addons = $model->addons;
        $newProduct->upgrades = $model->upgrades;
        $newProduct->config = $model->config;
        $newProduct->type = $model->type;
        $newProduct->icon_url = $model->icon_url;
        $newProduct->allow_quantity_select = $model->allow_quantity_select;
        $newProduct->quantity_in_stock = $model->quantity_in_stock;
        $newProduct->stock_control = $model->stock_control;
        $newProduct->plugin = $model->plugin;
        $newProduct->plugin_config = $model->plugin_config;
        $newProduct->priority = $model->priority + 1;
        $newProduct->updated_at = date('Y-m-d H:i:s');
        $newProduct->created_at = date('Y-m-d H:i:s');

        $productId = $this->di['db']->store($newProduct);
        $this->di['logger']->info('Copied product #%s to #%s', $model->id, $productId);
        return (int) $productId;
    }
    ...
    public function updateConfig(\Model_Product $model, $data)
    {
        ...
        if (isset($data['new_config_name']) && isset($data['new_config_value']) && !empty($data['new_config_name']) && !empty($data['new_config_value'])) {
    ...
    {
        $sql = 'SELECT id, title FROM product WHERE is_addon =1 ORDER by id asc';
    ...
    {
        $sql = 'SELECT m.* FROM product as m LEFT JOIN product_payment as pp on m.product_payment_id = pp.id WHERE m.is_addon = 0';
        $type = $data['type'] ?? null;
    ...
    {
        $sql = 'SELECT m.id, m.title, m.description, m.icon_url, m.created_at, m.updated_at, Max(p.priority) AS MaxPrio
                FROM product_category AS m LEFT JOIN product p ON p.product_category_id = m.id
                WHERE p.status = \'enabled\' AND p.hidden = 0
    ...
    public function getProductTitlesByIds($ids): array
    {
        ...
        $slots = (is_countable($ids) ? count($ids) : 0) ? implode(',', array_fill(0, is_countable($ids) ? count($ids) : 0, '?')) : '';
    ...
    public function getAllCategoryProducts(\Model_ProductCategory $model)
    {
        return $this->di['db']->find('Product', 'is_addon = 0 and product_category_id = ?', [$model->id]);
    }
    ...
    {
        $sql = 'SELECT min(price_registration) FROM tld WHERE active = 1';
    ...
    public function getStartingPrice(\Model_ProductPayment $model)
    {
        ...
        if ($model->type == 'recurrent') {
            $p = [];
            if ($model->w_enabled) { $p[] = $model->w_price; }
            if ($model->m_enabled) { $p[] = $model->m_price; }
            if ($model->q_enabled) { $p[] = $model->q_price; }
            if ($model->b_enabled) { $p[] = $model->b_price; }
            if ($model->a_enabled) { $p[] = $model->a_price; }
            if ($model->bia_enabled) { $p[] = $model->bia_price; }
            if ($model->tria_enabled) { $p[] = $model->tria_price; }
            if ($p) {
                return min($p);
            } else {
                return null;
    ...
    public function getProductAddons(\Model_Product $model)
    {
        ...
        $slots = (is_countable($ids) ? count($ids) : 0) ? implode(',', array_fill(0, is_countable($ids) ? count($ids) : 0, '?')) : '';
        array_unshift($ids, (int) $model->id);
    ...
    {
        $categories = $this->di['db']->find('ProductCategory', 'ORDER BY id ASC');
        $result = [];
        foreach ($categories as $cat) {
            $products = $this->di['db']->find('Product', 'is_addon = 0 AND product_category_id = ? ORDER BY priority ASC', [$cat->id]);
            $result[] = [
                'category' => $cat,
                'products' => $products,
            ];
        }
        $uncategorized = $this->di['db']->find('Product', 'is_addon = 0 AND (product_category_id IS NULL OR product_category_id = 0) ORDER BY priority ASC');
